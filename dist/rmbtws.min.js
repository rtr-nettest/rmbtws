 ...
 *****************************************************************************!*/
"use strict";var RMBTTest=function(){function e(e){v=e}function t(e){void 0!==M&&M===e||(M=e,S=performance.now())}function n(e,n,r){var i=(e.test_server_encryption?"wss://":"ws://")+e.test_server_address+":"+e.test_server_port;w(i);var u=function(){return{IGNORE:function(){},CALLGLOBALHANDLER:function(){C(RMBTError.CONNECT_FAILED)},TRYRECONNECT:function(){C(RMBTError.CONNECT_FAILED)}}}();n.onStateEnter(TestState.INIT_DOWN,function(){t(TestState.INIT_DOWN),w(n.id+": start short download"),0===n.id?o(n,v.pretestDurationMs):n.triggerNextState()}),n.onStateEnter(TestState.PING,function(){t(TestState.PING),w(n.id+": starting ping"),0===n.id?a(n):n.triggerNextState()}),n.onStateEnter(TestState.DOWN,function(){t(TestState.DOWN),n.id<B?d(n,e.test_duration):n.triggerNextState()}),n.onStateEnter(TestState.INIT_UP,function(){t(TestState.INIT_UP),g=m,0===n.id?c(n,v.pretestDurationMs):n.triggerNextState()}),n.onStateEnter(TestState.UP,function(){t(TestState.UP),n.id<L?l(n,e.test_duration):(n.socket.onerror=u.IGNORE,n.triggerNextState())}),n.onStateEnter(TestState.END,function(){0===n.id&&r()}),n.setState(TestState.INIT),t(TestState.INIT),s(n,i,e.test_token,u.CALLGLOBALHANDLER)}function s(e,t,n,s){try{e.socket=new WebSocket(t)}catch(e){return void C(RMBTError.SOCKET_INIT_FAILED)}e.socket.binaryType="arraybuffer",e.socket.onerror=s,e.socket.onmessage=function(t){if(0===t.data.indexOf("CHUNKSIZE"))g=parseInt(t.data.substring(10)),m=g,w(e.id+"Chunksize: "+g);else if(0===t.data.indexOf("RMBTv")){var o=t.data.substring(5).trim();v.client_version=o,0===o.indexOf("1.")?k=!0:0===o.indexOf("0.3")?k=!1:w("unknown server version: "+o)}else"ACCEPT TOKEN QUIT\n"===t.data?e.socket.send("TOKEN "+n+"\n"):"OK\n"===t.data&&e.state===TestState.INIT?w(e.id+": Token accepted"):"ERR\n"===t.data?(s(),w("got error msg")):0===t.data.indexOf("ACCEPT GETCHUNKS")&&e.triggerNextState()}}function o(e,t){var n=e.socket.onmessage,s=performance.now(),o=1,a=0;!function i(){r(e,o,function(r){a+=o*g,w(e.id+": "+r);var d=parseInt(r.substring(5));if(performance.now()-s>t){I=o*g/(d/1e9),w(e.id+": circa "+I/1e3+" KB/sec"),w(e.id+": circa "+8*I/1e6+" MBit/sec");var c=8*I/1e6;Object.keys(v.downloadThreadsLimitsMbit).forEach(function(e){c>e&&(B=v.downloadThreadsLimitsMbit[e])}),B=Math.min(h,B),w(e.id+": set number of threads to be used in download speed test to: "+B);var u=I/(1e3/(v.measurementPointsTimespan/2));u-=u%1024,u=Math.max(m,u),u=Math.min(N,u),w(e.id+": calculated chunksize for download speed test "+u/1024+" KB"),g=u,e.socket.onmessage=n}else o<8||!k?(o*=2,i()):(g=Math.min(2*g,N),i())})}()}function r(e,t,n){var s=e.socket,o=t,r=g*t,a=0,i=function(e){"string"!=typeof e.data&&(a+=e.data.byteLength,e.data.byteLength!==g&&1!==e.data.byteLength||o--,0===o?(s.onmessage=function(e){var t=e.data;n(t)},s.send("OK\n"),U[g]=e.data):(P.hasOwnProperty(g)||(P[g]=[]),P[g].length<v.savedChunks&&P[g].push(e.data)))};s.onmessage=i,w(e.id+": downloading "+t+" chunks, "+r/1e3+" KB");var d="GETCHUNKS "+t+(g!==m?" "+g:"")+"\n";s.send(d)}function a(e){var t=1/0,n=e.socket.onmessage,s=v.numPings;i(e,function o(r){if(s--,e.result.pings.push(r),r.client<t&&(t=r.client),w(e.id+": PING "+r.client+" ns client; "+r.server+" ns server"),s>0)e.socket.onmessage=function(t){"ACCEPT GETCHUNKS GETTIME PUT PUTNORESULT PING QUIT\n"===t.data?i(e,o):w("unexpected error during ping test")};else{for(var a=[],d=0;d<e.result.pings.length;d++)a.push(e.result.pings[d].client);E.ping_median=Math.median(a),w(e.id+": shortest: "+Math.round(t/1e3)/1e3+" ms"),E.ping_shortest=t,e.socket.onmessage=n}})}function i(e,t){var n,s,o=function(o){if("PONG\n"===o.data){var r=nowNs();s=r-n,e.socket.send("OK\n")}else if(0===o.data.indexOf("TIME")){var a=new RMBTPingResult;a.client=s,a.server=parseInt(o.data.substring(5)),a.timeNs=n,t(a)}};e.socket.onmessage=o,n=nowNs(),e.socket.send("PING\n")}function d(e,t){var n,s,o=e.socket.onmessage,r=0,a=0,i=-1,d=null;n=window.setInterval(function(){if(null!==d&&i!==a){i=a;var t=nowNs();w(e.id+": "+s+"|"+v.measurementPointsTimespan+"|"+t+"|"+a);var c=new Uint8Array(d,d.byteLength-1,1),t=nowNs(),l=t-u;e.result.down.push({duration:l,bytes:r}),s=t,c[0]>=255&&(w(e.id+": received end chunk"),window.clearInterval(n),e.socket.onmessage=function(t){w(t.data),e.socket.onmessage=o},e.socket.send("OK\n"))}},v.measurementPointsTimespan);var c=function(e){a++,r+=e.data.byteLength,d=e.data};e.socket.onmessage=c;var u=nowNs();e.socket.send("GETTIME "+t+(g!==m?" "+g:"")+"\n")}function c(e,t){var n=e.socket.onmessage,s=performance.now(),o=1,r=0;window.setTimeout(function(){var e=performance.now(),n=e-s;w("diff:"+(n-t)+" ("+(n-t)/t+" %)")},t);!function a(){u(e,o,function(i){if(r+=o*g,w(e.id+": "+i),performance.now()-s>t){e.socket.onmessage=n;var d=parseInt(i.substring(5));I=o*g/(d/1e9),w(e.id+": circa "+I/1e3+" KB/sec up"),w(e.id+": circa "+8*I/1e6+" MBit/sec up");var c=8*I/1e6;Object.keys(v.uploadThreadsLimitsMbit).forEach(function(e){c>e&&(L=v.uploadThreadsLimitsMbit[e])}),L=Math.min(h,L),w(e.id+": set number of threads to be used in upload speed test to: "+L);var u=I/(1e3/(v.measurementPointsTimespan/2));u-=u%1024,u=Math.max(m,u),u=Math.min(N,u);var l=Number.POSITIVE_INFINITY;Object.keys(P).forEach(function(e){Math.abs(u-e)<Math.abs(u-l)?l=e:(delete P[e],delete U[e])}),w(e.id+": calculated chunksize for upload speed test "+u/1024+" KB"),w(e.id+": used chunk size for upload speed test will be: "+l/1024+" KB"),g=l}else{var f=2*g;o<8||!U.hasOwnProperty(f)||!k?(o*=2,a()):(g=Math.min(2*g,N),a())}})}()}function u(e,t,n){var s=e.socket;s.onmessage=function(e){0!==e.data.indexOf("OK")&&0!==e.data.indexOf("ACCEPT")&&n(e.data)},w(e.id+": uploading "+t+" chunks, "+g*t/1e3+" KB"),s.send("PUTNORESULT"+(k?" "+g:"")+"\n");for(var o=0;o<t;o++){var r;r=o===t-1?U[g]:P[g][0],s.send(r)}}function l(e,t){var n=e.socket.onmessage,s=I/2/L,o=Math.ceil(I/L/g),r=!1,a=!0,i=-1,d=0,c=function s(){r||(w(e.id+": is 7.2 sec in, got data for "+i),i<1e9*t&&d<3e3?(window.setTimeout(s,250),d+=250):(w(e.id+": didn't finish, timeout extended by "+d+" ms, last info for "+i),e.socket.onerror=function(){},e.socket.close(),e.socket.onmessage=n,w(e.id+": socket now closed: "+e.socket.readyState),e.triggerNextState()))},u=function t(){if(e.socket.bufferedAmount<s)for(var n=0;n<o;n++)e.socket.send(P[g][n%P[g].length]);if(!a)return!1;setTimeout(t,0)};window.setTimeout(c,1e3*t+200),window.setTimeout(function(){a=!1,e.socket.send(U[g]),e.socket.send("QUIT\n")},1e3*t),w(e.id+": set timeout");var l=/TIME (\d+) BYTES (\d+)/,f=/TIME (\d+)/,p=function(t){"OK\n"===t.data&&u();var s=l.exec(t.data);if(null!==s){var o={duration:parseInt(s[1]),bytes:parseInt(s[2])};i=o.duration,e.result.up.push(o)}else null!==(s=f.exec(t.data))&&(r=!0,w("Upload duration: "+s[1]),e.socket.onmessage=n)};e.socket.onmessage=p,e.socket.send("PUT"+(g!==m?" "+g:"")+"\n")}function f(e,t){var n={version:e.version,language:e.language,uuid:e.uuid,type:e.type,version_code:e.version_code,client:e.client,timezone:e.timezone,time:(new Date).getTime()};"undefined"!=typeof userServerSelection&&userServerSelection>0&&"undefined"!=typeof UserConf&&void 0!==UserConf.preferredServer&&"default"!==UserConf.preferredServer&&(n.prefer_server=UserConf.preferredServer,n.user_server_selection=userServerSelection),$.ajax({url:e.controlServerURL+e.controlServerRegistrationResource,type:"post",dataType:"json",contentType:"application/json",data:JSON.stringify(n),success:function(e){var n=new RMBTControlServerRegistrationResponse(e);t(n)},error:function(){w("error getting testID")}})}function p(e){$.ajax({url:e.controlServerURL+e.controlServerDataCollectorResource,type:"get",dataType:"json",contentType:"application/json",success:function(t){e.product=t.agent.substring(0,Math.min(150,t.agent.length)),e.model=t.product,e.os_version=t.version},error:function(){w("error getting data collection response")}})}function T(e,t){var n={client_language:"de",client_name:v.client,client_uuid:v.uuid,client_version:v.client_version,client_software_version:v.client_software_version,geoLocations:E.geoLocations,model:v.model,network_type:98,platform:v.platform,product:v.product,pings:E.pings,test_bytes_download:E.bytes_download,test_bytes_upload:E.bytes_upload,test_nsec_download:E.nsec_download,test_nsec_upload:E.nsec_upload,test_num_threads:B,num_threads_ul:L,test_ping_shortest:E.ping_shortest,test_speed_download:E.speed_download,test_speed_upload:E.speed_upload,test_token:e.test_token,time:E.beginTime,timezone:"Europe/Vienna",type:"DESKTOP",version_code:"1",speed_detail:E.speedItems,user_server_selection:v.userServerSelection},s=JSON.stringify(n);w("Submit size: "+s.length),$.ajax({url:v.controlServerURL+v.controlServerResultResource,type:"post",dataType:"json",contentType:"application/json",data:s,success:function(n){w("https://develop.netztest.at/en/Verlauf?"+e.test_uuid),t()},error:function(){w("error submitting results")}})}var g,m,v,S,_,h,I,w=function(e){$("#debug").prepend(e+"\n"),console.log(e)},N=4194304,k=!1,E=null,b=null,M=TestState.INIT,y={durationInitMs:2500,durationPingMs:500,durationUpMs:-1,durationDownMs:-1},O=new RMBTIntermediateResult,R=new Array,P={},U={},B=0,L=0;e.prototype.onError=function(e){b=e};var C=function(e){if(!e===RMBTError.NOT_SUPPORTED&&t(TestState.ERROR),null!==b){var n=b;b=null,n()}};return e.prototype.startTest=function(){if(void 0===window.WebSocket)return void C(RMBTError.NOT_SUPPORTED);t(TestState.INIT),E=new RMBTTestResult,p(v),f(v,function(e){h=parseInt(e.test_numthreads),_=new CyclicBarrier(h),y.durationDownMs=1e3*e.test_duration,y.durationUpMs=1e3*e.test_duration,null!==TestEnvironment.getTestVisualization()&&TestEnvironment.getTestVisualization().updateInfo(e.test_server_name,e.client_remote_ip,e.provider,e.test_uuid);var s,o=function(){w("got geolocation, obtaining token and websocket address"),t(TestState.WAIT),window.setTimeout(function(){t(TestState.INIT),E.beginTime=Date().now;for(var o=0;o<h;o++){var r=new RMBTTestThread(_);r.id=o,E.addThread(r.result),n(e,r,function(){w("All tests finished"),s.stop(),E.geoLocations=s.getResults(),E.calculateAll(),T(e,function(){t(TestState.END)})}),R.push(r)}},1e3*e.test_wait)};null!==TestEnvironment.getGeoTracker()?(s=TestEnvironment.getGeoTracker(),o()):(s=new GeoTracker,w("getting geolocation"),s.start(function(){o()}))})},e.prototype.getIntermediateResult=function(){O.status=M;var e=performance.now()-S;switch(O.status){case TestState.WAIT:O.progress=0;break;case TestState.INIT:case TestState.WAIT:case TestState.INIT_DOWN:case TestState.INIT_UP:O.progress=e/y.durationInitMs;break;case TestState.PING:O.progress=e/y.durationPingMs;break;case TestState.DOWN:O.progress=e/y.durationDownMs;break;case TestState.UP:O.progress=e/y.durationUpMs;break;case TestState.END:O.progress=1;break;case TestState.ERROR:case TestState.ABORTED:O.progress=0}if(isNaN(O.progress)&&(O.progress=0),O.progress=Math.min(1,O.progress),null!==E){if(O.pingNano=E.ping_median,O.status===TestState.DOWN){for(var t=0,n=1/0,s=0;s<R.length>0;s++){var o=E.threads[s].down;o.length>0&&(t+=o[o.length-1].bytes,o[o.length-1].duration<n&&(n=o[o.length-1].duration))}O.downBitPerSec=Math.max(0,8*t/(n/1e9)),O.downBitPerSecLog=(log10(O.downBitPerSec/1e6)+2)/4}if(O.status===TestState.UP){t=0,n=1/0;for(var s=0;s<R.length;s++){var r=E.threads[s].up;r.length>0&&(t+=r[r.length-1].bytes,r[r.length-1].duration<n&&(n=r[r.length-1].duration))}O.upBitPerSec=Math.max(0,8*t/(n/1e9)),O.upBitPerSecLog=(log10(O.upBitPerSec/1e6)+2)/4}}return O},e.prototype.getState=function(){return"INIT"},e}();
