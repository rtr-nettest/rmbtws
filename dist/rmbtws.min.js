 ...
 *****************************************************************************!*/
"use strict";var RMBTTest=function(){function e(e){_=e,h=TestState.INIT}function t(e){void 0!==h&&h===e||(h=e,N=performance.now())}function n(e,n,a){var u=(e.test_server_encryption?"wss://":"ws://")+e.test_server_address+":"+e.test_server_port;w(u);var l=function(){return{IGNORE:function(){},CALLGLOBALHANDLER:function(){A(RMBTError.CONNECT_FAILED)},TRYRECONNECT:function(){A(RMBTError.CONNECT_FAILED)}}}();n.onStateEnter(TestState.INIT_DOWN,function(){t(TestState.INIT_DOWN),w(n.id+": start short download"),v=m,r(n,_.pretestDurationMs)}),n.onStateEnter(TestState.PING,function(){t(TestState.PING),w(n.id+": starting ping"),0===n.id?i(n):n.triggerNextState()}),n.onStateEnter(TestState.DOWN,function(){if(t(TestState.DOWN),B.length>0){var s=o(B,_.downloadThreadsLimitsMbit,!1);L=s.numThreads,v=s.chunkSize,B=[]}n.id<L?c(n,e.test_duration):n.triggerNextState()}),n.onStateEnter(TestState.INIT_UP,function(){t(TestState.INIT_UP),v=m,d(n,_.pretestDurationMs)}),n.onStateEnter(TestState.UP,function(){if(t(TestState.UP),B.length>0){var s=o(B,_.uploadThreadsLimitsMbit,!0);D=s.numThreads,v=s.chunkSize,B=[]}n.id<D?f(n,e.test_duration):(n.socket.onerror=l.IGNORE,n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),n.triggerNextState())}),n.onStateEnter(TestState.END,function(){n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),0===n.id&&a()}),n.setState(TestState.INIT),t(TestState.INIT),s(n,u,e.test_token,l.CALLGLOBALHANDLER)}function s(e,t,n,s){try{e.socket=new WebSocket(t)}catch(e){return void A(RMBTError.SOCKET_INIT_FAILED)}e.socket.binaryType="arraybuffer",e.socket.onerror=s,e.socket.onmessage=function(t){if(0===t.data.indexOf("CHUNKSIZE")){var o=t.data.trim().split(" ");4===o.length?(S=parseInt(o[1]),m=parseInt(o[2]),E=parseInt(o[3])):(S=parseInt(o[1]),m=S),w(e.id+"Chunksizes: min "+m+", max: "+E+", default: "+S)}else if(0===t.data.indexOf("RMBTv")){var r=t.data.substring(5).trim();_.client_version=r,0===r.indexOf("1.")?y=!0:0===r.indexOf("0.3")?y=!1:w("unknown server version: "+r)}else"ACCEPT TOKEN QUIT\n"===t.data?e.socket.send("TOKEN "+n+"\n"):"OK\n"===t.data&&e.state===TestState.INIT?w(e.id+": Token accepted"):"ERR\n"===t.data?(s(),w("got error msg")):0===t.data.indexOf("ACCEPT GETCHUNKS")&&e.triggerNextState()}}function o(e,t,n){x=e.reduce(function(e,t){return e+t}),w("total: circa "+x/1e3+" KB/sec"),w("total: circa "+8*x/1e6+" MBit/sec");var s=8*x/1e6,o=0;Object.keys(t).forEach(function(e){s>e&&(o=t[e])}),o=Math.min(k,o),w("set number of threads to be used in upcoming speed test to: "+o);var r=x/(1e3/(_.measurementPointsTimespan/2));if(r-=r%1024,r=Math.max(m,r),r=Math.min(E,r),w("calculated chunksize for upcoming speed test "+r/1024+" KB"),n){var a=Number.POSITIVE_INFINITY;Object.keys(U).forEach(function(e){Math.abs(r-e)<Math.abs(r-a)?a=e:(delete U[e],delete C[e])}),r=a,w("fallback to existing chunksize for upcoming speed test "+r/1024+" KB")}return{numThreads:o,chunkSize:r,bytesPerSecs:x}}function r(e,t){var n=e.socket.onmessage,s=nowNs()/1e6,o=1,r=0,i=v;!function u(){a(e,o,i,function(a){r+=o*i,w(e.id+": "+a);var c=parseInt(a.substring(5));nowNs()/1e6-s>t?(B.push(o*i/(c/1e9)),e.socket.onmessage=n):o<8||!y?(o*=2,u()):(i=Math.min(2*i,E),u())})}()}function a(e,t,n,s){var o=e.socket,r=t,a=v*t,i=0,u=function(e){"string"!=typeof e.data&&(i+=e.data.byteLength,e.data.byteLength!==n&&1!==e.data.byteLength||r--,0===r?(o.onmessage=function(e){var t=e.data;s(t)},o.send("OK\n"),C[n]=e.data):(U.hasOwnProperty(n)||(U[n]=[]),U[n].length<_.savedChunks&&U[n].push(e.data)))};o.onmessage=u,w(e.id+": downloading "+t+" chunks, "+a/1e3+" KB");var c="GETCHUNKS "+t+(n!==S?" "+n:"")+"\n";o.send(c)}function i(e){var t=1/0,n=e.socket.onmessage,s=_.numPings;u(e,function o(r){if(s--,e.result.pings.push(r),r.client<t&&(t=r.client),w(e.id+": PING "+r.client+" ns client; "+r.server+" ns server"),s>0)e.socket.onmessage=function(t){"ACCEPT GETCHUNKS GETTIME PUT PUTNORESULT PING QUIT\n"===t.data?u(e,o):w("unexpected error during ping test")};else{for(var a=[],i=0;i<e.result.pings.length;i++)a.push(e.result.pings[i].client);O.ping_median=Math.median(a),w(e.id+": shortest: "+Math.round(t/1e3)/1e3+" ms"),O.ping_shortest=t,e.socket.onmessage=n}})}function u(e,t){var n,s,o=function(o){if("PONG\n"===o.data){var r=nowNs();s=r-n,e.socket.send("OK\n")}else if(0===o.data.indexOf("TIME")){var a=new RMBTPingResult;a.client=s,a.server=parseInt(o.data.substring(5)),a.timeNs=n,t(a)}};e.socket.onmessage=o,n=nowNs(),e.socket.send("PING\n")}function c(e,t){var n,s,o=e.socket.onmessage,r=0,a=0,i=-1,u=null,c=null;n=window.setInterval(function(){if(null!==u&&i!==a){i=a;var t=nowNs();w(e.id+": "+s+"|"+_.measurementPointsTimespan+"|"+t+"|"+a);var d=new Uint8Array(u,u.byteLength-1,1),f=c-l;e.result.down.push({duration:f,bytes:r}),s=t,d[0]>=255&&(w(e.id+": received end chunk"),window.clearInterval(n),e.socket.onmessage=function(t){w(t.data),e.socket.onmessage=o},e.socket.send("OK\n"))}},_.measurementPointsTimespan);var d=function(e){a++,r+=e.data.byteLength,c=nowNs(),u=e.data};e.socket.onmessage=d;var l=nowNs();e.socket.send("GETTIME "+t+(v!==S?" "+v:"")+"\n")}function d(e,t){var n=e.socket.onmessage,s=nowNs()/1e6,o=1,r=0,a=v;window.setTimeout(function(){var e=nowNs()/1e6,n=e-s;w("diff:"+(n-t)+" ("+(n-t)/t+" %)")},t);!function i(){l(e,o,a,function(u){if(r+=o*a,w(e.id+": "+u),nowNs()/1e6-s>t){e.socket.onmessage=n;var c=parseInt(u.substring(5));B.push(o*a/(c/1e9))}else{var d=2*a;o<8||!C.hasOwnProperty(d)||!y?(o*=2,i()):(a=Math.min(2*a,E),i())}})}()}function l(e,t,n,s){var o=e.socket;o.onmessage=function(e){0!==e.data.indexOf("OK")&&0!==e.data.indexOf("ACCEPT")&&s(e.data)},w(e.id+": uploading "+t+" chunks, "+n*t/1e3+" KB"),o.send("PUTNORESULT"+(y?" "+n:"")+"\n");for(var r=0;r<t;r++){var a;a=r===t-1?C[n]:U[n][0],o.send(a)}}function f(e,t){var n=e.socket.onmessage,s=x/2/D,o=Math.ceil(x/D/v),r=!1,a=!0,i=-1,u=0,c=function s(){r||(w(e.id+": is 7.2 sec in, got data for "+i),i<1e9*t&&u<3e3?(window.setTimeout(s,250),u+=250):(w(e.id+": didn't finish, timeout extended by "+u+" ms, last info for "+i),e.socket.onerror=function(){},e.socket.close(),e.socket.onmessage=n,w(e.id+": socket now closed: "+e.socket.readyState),e.triggerNextState()))},d=function t(){if(e.socket.bufferedAmount<s)for(var n=0;n<o;n++)e.socket.send(U[v][n%U[v].length]);if(!a)return!1;setTimeout(t,0)};window.setTimeout(c,1e3*t+200),window.setTimeout(function(){a=!1,e.socket.send(C[v]),e.socket.send("QUIT\n")},1e3*t),w(e.id+": set timeout");var l=/TIME (\d+) BYTES (\d+)/,f=/TIME (\d+)/,g=function(t){"OK\n"===t.data&&d();var s=l.exec(t.data);if(null!==s){var o={duration:parseInt(s[1]),bytes:parseInt(s[2])};i=o.duration,e.result.up.push(o)}else null!==(s=f.exec(t.data))&&(r=!0,w("Upload duration: "+s[1]),e.socket.onmessage=n)};e.socket.onmessage=g,e.socket.send("PUT"+(v!==S?" "+v:"")+"\n")}function g(e,t){var n={version:e.version,language:e.language,uuid:e.uuid,type:e.type,version_code:e.version_code,client:e.client,timezone:e.timezone,time:(new Date).getTime()};"undefined"!=typeof userServerSelection&&userServerSelection>0&&"undefined"!=typeof UserConf&&void 0!==UserConf.preferredServer&&"default"!==UserConf.preferredServer&&(n.prefer_server=UserConf.preferredServer,n.user_server_selection=userServerSelection),$.ajax({url:e.controlServerURL+e.controlServerRegistrationResource,type:"post",dataType:"json",contentType:"application/json",data:JSON.stringify(n),success:function(e){var n=new RMBTControlServerRegistrationResponse(e);t(n)},error:function(){w("error getting testID")}})}function T(e){$.ajax({url:e.controlServerURL+e.controlServerDataCollectorResource,type:"get",dataType:"json",contentType:"application/json",success:function(t){e.product=t.agent.substring(0,Math.min(150,t.agent.length)),e.model=t.product,e.os_version=t.version},error:function(){w("error getting data collection response")}})}function p(e,t){var n={client_language:"de",client_name:_.client,client_uuid:_.uuid,client_version:_.client_version,client_software_version:_.client_software_version,geoLocations:O.geoLocations,model:_.model,network_type:98,platform:_.platform,product:_.product,pings:O.pings,test_bytes_download:O.bytes_download,test_bytes_upload:O.bytes_upload,test_nsec_download:O.nsec_download,test_nsec_upload:O.nsec_upload,test_num_threads:L,num_threads_ul:D,test_ping_shortest:O.ping_shortest,test_speed_download:O.speed_download,test_speed_upload:O.speed_upload,test_token:e.test_token,time:O.beginTime,timezone:"Europe/Vienna",type:"DESKTOP",version_code:"1",speed_detail:O.speedItems,user_server_selection:_.userServerSelection},s=JSON.stringify(n);w("Submit size: "+s.length),$.ajax({url:_.controlServerURL+_.controlServerResultResource,type:"post",dataType:"json",contentType:"application/json",data:s,success:function(n){w("https://develop.netztest.at/en/Verlauf?"+e.test_uuid),t()},error:function(){w("error submitting results")}})}var v,m,S,_,h,N,I,k,w=function(e){$("#debug").prepend(e+"\n"),console.log(e)},E=4194304,y=!1,O=null,b=null,M={durationInitMs:2500,durationPingMs:500,durationUpMs:-1,durationDownMs:-1},R=new RMBTIntermediateResult,P=[],U={},C={},L=0,D=0,B=[],x=0;e.prototype.onError=function(e){b=e};var A=function(e){if(!e===RMBTError.NOT_SUPPORTED&&t(TestState.ERROR),null!==b){var n=b;b=null,n()}};return e.prototype.startTest=function(){if(void 0===window.WebSocket)return void A(RMBTError.NOT_SUPPORTED);t(TestState.INIT),O=new RMBTTestResult,T(_),g(_,function(e){k=parseInt(e.test_numthreads),I=new CyclicBarrier(k),M.durationDownMs=1e3*e.test_duration,M.durationUpMs=1e3*e.test_duration,null!==TestEnvironment.getTestVisualization()&&TestEnvironment.getTestVisualization().updateInfo(e.test_server_name,e.client_remote_ip,e.provider,e.test_uuid);var s,o=function(){w("got geolocation, obtaining token and websocket address"),t(TestState.WAIT),window.setTimeout(function(){t(TestState.INIT),O.beginTime=Date.now();for(var o=0;o<k;o++){var r=new RMBTTestThread(I);r.id=o,O.addThread(r.result),n(e,r,function(){w("All tests finished"),s.stop(),O.geoLocations=s.getResults(),O.calculateAll(),p(e,function(){t(TestState.END)})}),P.push(r)}},1e3*e.test_wait)};null!==TestEnvironment.getGeoTracker()?(s=TestEnvironment.getGeoTracker(),o()):(s=new GeoTracker,w("getting geolocation"),s.start(function(){o()}))})},e.prototype.getIntermediateResult=function(){R.status=h;var e=nowNs()/1e6-N;switch(R.status){case TestState.WAIT:R.progress=0;break;case TestState.INIT:case TestState.INIT_DOWN:case TestState.INIT_UP:R.progress=e/M.durationInitMs;break;case TestState.PING:R.progress=e/M.durationPingMs;break;case TestState.DOWN:R.progress=e/M.durationDownMs;break;case TestState.UP:R.progress=e/M.durationUpMs;break;case TestState.END:R.progress=1;break;case TestState.ERROR:case TestState.ABORTED:R.progress=0}if(isNaN(R.progress)&&(R.progress=0),R.progress=Math.min(1,R.progress),null!==O){if(R.pingNano=O.ping_median,R.status===TestState.DOWN){for(var t=0,n=1/0,s=0;s<P.length>0;s++){var o=O.threads[s].down;o.length>0&&(t+=o[o.length-1].bytes,o[o.length-1].duration<n&&(n=o[o.length-1].duration))}R.downBitPerSec=Math.max(0,8*t/(n/1e9)),R.downBitPerSecLog=(log10(R.downBitPerSec/1e6)+2)/4}if(R.status===TestState.UP){t=0,n=1/0;for(var s=0;s<P.length;s++){var r=O.threads[s].up;r.length>0&&(t+=r[r.length-1].bytes,r[r.length-1].duration<n&&(n=r[r.length-1].duration))}R.upBitPerSec=Math.max(0,8*t/(n/1e9)),R.upBitPerSecLog=(log10(R.upBitPerSec/1e6)+2)/4}}return R},e.prototype.getState=function(){return"INIT"},e}();
