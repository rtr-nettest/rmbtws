 ...
 *****************************************************************************!*/
"use strict";var RMBTTest=function(){function e(e,t){k=e,I=t,w=TestState.INIT}function t(e){void 0!==w&&w===e||(w=e,E=nowMs())}function n(e,n,r){var d=(e.test_server_encryption?"wss://":"ws://")+e.test_server_address+":"+e.test_server_port;g(d);var l=function(){return{IGNORE:function(){},CALLGLOBALHANDLER:function(){x(RMBTError.CONNECT_FAILED)},TRYRECONNECT:function(){x(RMBTError.CONNECT_FAILED)}}}();n.onStateEnter(TestState.INIT_DOWN,function(){t(TestState.INIT_DOWN),g(n.id+": start short download"),p=m,a(n,k.pretestDurationMs)}),n.onStateEnter(TestState.PING,function(){t(TestState.PING),g(n.id+": starting ping"),0===n.id?i(n):n.triggerNextState()}),n.onStateEnter(TestState.DOWN,function(){if(t(TestState.DOWN),D.length>0){var s=o(D,k.downloadThreadsLimitsMbit,!1);L=s.numThreads,p=s.chunkSize,D=[]}n.id<L?u(n,e.test_duration):n.triggerNextState()}),n.onStateEnter(TestState.INIT_UP,function(){t(TestState.INIT_UP),p=m,c(n,k.pretestDurationMs)}),n.onStateEnter(TestState.UP,function(){if(t(TestState.UP),D.length>0){var s=o(D,k.uploadThreadsLimitsMbit,!0);C=s.numThreads,p=s.chunkSize,D=[]}n.id<C?T(n,e.test_duration):(n.socket.onerror=l.IGNORE,n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),n.triggerNextState())}),n.onStateEnter(TestState.END,function(){n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),0===n.id&&r()}),n.setState(TestState.INIT),t(TestState.INIT),s(n,d,e.test_token,l.CALLGLOBALHANDLER)}function s(e,t,n,s){try{e.socket=new WebSocket(t)}catch(e){return void x(RMBTError.SOCKET_INIT_FAILED)}e.socket.binaryType="arraybuffer",e.socket.onerror=s,e.socket.onmessage=function(t){if(0===t.data.indexOf("CHUNKSIZE")){var o=t.data.trim().split(" ");4===o.length?(_=parseInt(o[1]),m=parseInt(o[2]),v=parseInt(o[3])):(_=parseInt(o[1]),m=_),g(e.id+"Chunksizes: min "+m+", max: "+v+", default: "+_)}else if(0===t.data.indexOf("RMBTv")){var a=t.data.substring(5).trim();k.client_version=a,0===a.indexOf("1.")?S=!0:0===a.indexOf("0.3")?S=!1:g("unknown server version: "+a)}else"ACCEPT TOKEN QUIT\n"===t.data?e.socket.send("TOKEN "+n+"\n"):"OK\n"===t.data&&e.state===TestState.INIT?g(e.id+": Token accepted"):"ERR\n"===t.data?(s(),g("got error msg")):0===t.data.indexOf("ACCEPT GETCHUNKS")&&e.triggerNextState()}}function o(e,t,n){U=e.reduce(function(e,t){return e+t}),g("total: circa "+U/1e3+" KB/sec"),g("total: circa "+8*U/1e6+" MBit/sec");var s=8*U/1e6,o=0;Object.keys(t).forEach(function(e){s>e&&(o=t[e])}),o=Math.min(B,o),g("set number of threads to be used in upcoming speed test to: "+o);var a=U/(1e3/(k.measurementPointsTimespan/2));if(a-=a%1024,a=Math.max(m,a),a=Math.min(v,a),g("calculated chunksize for upcoming speed test "+a/1024+" KB"),n){var r=Number.POSITIVE_INFINITY;Object.keys(P).forEach(function(e){Math.abs(a-e)<Math.abs(a-r)?r=e:(delete P[e],delete R[e])}),a=r,g("fallback to existing chunksize for upcoming speed test "+a/1024+" KB")}return{numThreads:o,chunkSize:a,bytesPerSecs:U}}function a(e,t){var n=e.socket.onmessage,s=nowMs(),o=1,a=0,i=p;!function d(){r(e,o,i,function(r){a+=o*i,g(e.id+": "+r);var u=parseInt(r.substring(5));nowMs()-s>t?(D.push(o*i/(u/1e9)),e.socket.onmessage=n):o<8||!S?(o*=2,d()):(i=Math.min(2*i,v),d())})}()}function r(e,t,n,s){var o=e.socket,a=t,r=p*t,i=0,d=function(e){"string"!=typeof e.data&&(i+=e.data.byteLength,e.data.byteLength!==n&&1!==e.data.byteLength||a--,0===a?(o.onmessage=function(e){var t=e.data;s(t)},o.send("OK\n"),R[n]=e.data):(P.hasOwnProperty(n)||(P[n]=[]),P[n].length<k.savedChunks&&P[n].push(e.data)))};o.onmessage=d,g(e.id+": downloading "+t+" chunks, "+r/1e3+" KB");var u="GETCHUNKS "+t+(n!==_?" "+n:"")+"\n";o.send(u)}function i(e){var t=1/0,n=e.socket.onmessage,s=k.numPings;d(e,function o(a){if(s--,e.result.pings.push(a),a.client<t&&(t=a.client),g(e.id+": PING "+a.client+" ns client; "+a.server+" ns server"),s>0)e.socket.onmessage=function(t){"ACCEPT GETCHUNKS GETTIME PUT PUTNORESULT PING QUIT\n"===t.data?d(e,o):g("unexpected error during ping test")};else{for(var r=[],i=0;i<e.result.pings.length;i++)r.push(e.result.pings[i].client);N.ping_median=Math.median(r),g(e.id+": shortest: "+Math.round(t/1e3)/1e3+" ms"),N.ping_shortest=t,e.socket.onmessage=n}})}function d(e,t){var n=void 0,s=void 0,o=function(o){if("PONG\n"===o.data){var a=nowNs();s=a-n,e.socket.send("OK\n")}else if(0===o.data.indexOf("TIME")){var r=new RMBTPingResult;r.client=s,r.server=parseInt(o.data.substring(5)),r.timeNs=n,t(r)}};e.socket.onmessage=o,n=nowNs(),e.socket.send("PING\n")}function u(e,t){var n=e.socket.onmessage,s=0,o=0,a=-1,r=void 0,i=void 0,d=null,u=null;r=window.setInterval(function(){if(null!==d&&a!==o){a=o;var t=nowNs();g(e.id+": "+i+"|"+k.measurementPointsTimespan+"|"+t+"|"+o);var c=new Uint8Array(d,d.byteLength-1,1),T=u-l;e.result.down.push({duration:T,bytes:s}),i=t,c[0]>=255&&(g(e.id+": received end chunk"),window.clearInterval(r),e.socket.onmessage=function(t){g(t.data),e.socket.onmessage=n},e.socket.send("OK\n"))}},k.measurementPointsTimespan);var c=function(e){o++,s+=e.data.byteLength,u=nowNs(),d=e.data};e.socket.onmessage=c;var l=nowNs();e.socket.send("GETTIME "+t+(p!==_?" "+p:"")+"\n")}function c(e,t){var n=e.socket.onmessage,s=nowMs(),o=1,a=0,r=p;window.setTimeout(function(){var e=nowMs(),n=e-s;g("diff:"+(n-t)+" ("+(n-t)/t+" %)")},t);!function i(){l(e,o,r,function(d){if(a+=o*r,g(e.id+": "+d),nowMs()-s>t){e.socket.onmessage=n;var u=parseInt(d.substring(5));D.push(o*r/(u/1e9))}else{var c=2*r;o<8||!R.hasOwnProperty(c)||!S?(o*=2,i()):(r=Math.min(2*r,v),i())}})}()}function l(e,t,n,s){var o=e.socket;o.onmessage=function(e){0!==e.data.indexOf("OK")&&0!==e.data.indexOf("ACCEPT")&&s(e.data)},g(e.id+": uploading "+t+" chunks, "+n*t/1e3+" KB"),o.send("PUTNORESULT"+(S?" "+n:"")+"\n");for(var a=0;a<t;a++){var r=void 0;r=a===t-1?R[n]:P[n][0],o.send(r)}}function T(e,t){var n=e.socket.onmessage,s=U/2/C,o=Math.ceil(U/C/p),a=!1,r=!0,i=-1,d=0,u=function s(){a||(g(e.id+": is 7.2 sec in, got data for "+i),i<1e9*t&&d<3e3?(window.setTimeout(s,250),d+=250):(g(e.id+": didn't finish, timeout extended by "+d+" ms, last info for "+i),e.socket.onerror=function(){},e.socket.close(),e.socket.onmessage=n,g(e.id+": socket now closed: "+e.socket.readyState),e.triggerNextState()))},c=function t(){if(e.socket.bufferedAmount<s)for(var n=0;n<o;n++)e.socket.send(P[p][n%P[p].length]);if(!r)return!1;setTimeout(t,0)};window.setTimeout(u,1e3*t+200),window.setTimeout(function(){r=!1,e.socket.send(R[p]),e.socket.send("QUIT\n")},1e3*t),g(e.id+": set timeout");var l=/TIME (\d+) BYTES (\d+)/,T=/TIME (\d+)/,f=function(t){"OK\n"===t.data&&c();var s=l.exec(t.data);if(null!==s){var o={duration:parseInt(s[1]),bytes:parseInt(s[2])};i=o.duration,e.result.up.push(o)}else null!==(s=T.exec(t.data))&&(a=!0,g("Upload duration: "+s[1]),e.socket.onmessage=n)};e.socket.onmessage=f,e.socket.send("PUT"+(p!==_?" "+p:"")+"\n")}function f(e){return console.debug(e.test_token),console.debug(e.test_token.split("_")),console.debug(e.test_token.split("_")[0]),{client_language:"de",client_name:k.client,client_uuid:k.uuid,client_version:k.client_version,client_software_version:k.client_software_version,geoLocations:N.geoLocations,model:k.model,network_type:98,platform:k.platform,product:k.product,pings:N.pings,test_bytes_download:N.bytes_download,test_bytes_upload:N.bytes_upload,test_nsec_download:N.nsec_download,test_nsec_upload:N.nsec_upload,test_num_threads:L,num_threads_ul:C,test_ping_shortest:N.ping_shortest,test_speed_download:N.speed_download,test_speed_upload:N.speed_upload,test_token:e.test_token,test_uuid:e.test_token.split("_")[0],time:N.beginTime,timezone:"Europe/Vienna",type:"DESKTOP",version_code:"1",speed_detail:N.speedItems,user_server_selection:k.userServerSelection}}var g=function(e){$("#debug").prepend(e+"\n"),console.log(e)},p=void 0,v=4194304,m=void 0,_=void 0,S=!1,k=void 0,I=void 0,N=null,h=null,w=void 0,E=void 0,M={durationInitMs:2500,durationPingMs:500,durationUpMs:-1,durationDownMs:-1},O=new RMBTIntermediateResult,b=[],P={},R={},y=void 0,B=void 0,L=0,C=0,D=[],U=0;e.prototype.onError=function(e){h=e};var x=function(e){if(!e===RMBTError.NOT_SUPPORTED&&t(TestState.ERROR),null!==h){var n=h;h=null,n()}};return e.prototype.startTest=function(){if(void 0===window.WebSocket)return void x(RMBTError.NOT_SUPPORTED);t(TestState.INIT),N=new RMBTTestResult,I.getDataCollectorInfo(k),I.obtainControlServerRegistration(k,function(e){B=parseInt(e.test_numthreads),y=new CyclicBarrier(B),M.durationDownMs=1e3*e.test_duration,M.durationUpMs=1e3*e.test_duration,null!==TestEnvironment.getTestVisualization()&&TestEnvironment.getTestVisualization().updateInfo(e.test_server_name,e.client_remote_ip,e.provider,e.test_uuid);var s=function(){g("got geolocation, obtaining token and websocket address"),t(TestState.WAIT),window.setTimeout(function(){t(TestState.INIT),N.beginTime=Date.now();for(var s=0;s<B;s++){var a=new RMBTTestThread(y);a.id=s,N.addThread(a.result),n(e,a,function(){g("All tests finished"),o.stop(),N.geoLocations=o.getResults(),N.calculateAll(),I.submitResults(k.controlServerURL+k.controlServerResultResource,f(e),function(){t(TestState.END)})}),b.push(a)}},1e3*e.test_wait)},o=void 0;null!==TestEnvironment.getGeoTracker()?(o=TestEnvironment.getGeoTracker(),s()):(o=new GeoTracker,g("getting geolocation"),o.start(function(){s()}))})},e.prototype.getIntermediateResult=function(){O.status=w;var e=nowNs()/1e6-E;switch(O.status){case TestState.WAIT:O.progress=0;break;case TestState.INIT:case TestState.INIT_DOWN:case TestState.INIT_UP:O.progress=e/M.durationInitMs;break;case TestState.PING:O.progress=e/M.durationPingMs;break;case TestState.DOWN:O.progress=e/M.durationDownMs;break;case TestState.UP:O.progress=e/M.durationUpMs;break;case TestState.END:O.progress=1;break;case TestState.ERROR:case TestState.ABORTED:O.progress=0}if(isNaN(O.progress)&&(O.progress=0),O.progress=Math.min(1,O.progress),null!==N){if(O.pingNano=N.ping_median,O.status===TestState.DOWN){var t=RMBTTestResult.calculateOverallSpeedFromMultipleThreads(N.threads,function(e){return e.down});O.downBitPerSec=t.speed,O.downBitPerSecLog=(log10(O.downBitPerSec/1e6)+2)/4}if(O.status===TestState.UP){var n=RMBTTestResult.calculateOverallSpeedFromMultipleThreads(N.threads,function(e){return e.up});O.upBitPerSec=n.speed,O.upBitPerSecLog=(log10(O.upBitPerSec/1e6)+2)/4}}return O},e.prototype.getState=function(){return"INIT"},e}();
