 ...
 *****************************************************************************!*/
"use strict";function RMBTTest(e,t){function n(e){void 0!==E&&E===e||(E=e,w=nowMs(),k&&k(e))}function s(e,t,s){var i=(e.test_server_encryption?"wss://":"ws://")+e.test_server_address+":"+e.test_server_port;i="ws://sdevv4-rmbtws.netztest.at:8081",f.debug(i);var d=function(){return{IGNORE:function(){},CALLGLOBALHANDLER:function(e){e&&f.error("connection closed",e),x(RMBTError.CONNECT_FAILED)},TRYRECONNECT:function(){x(RMBTError.CONNECT_FAILED)}}}();t.onStateEnter(TestState.INIT_DOWN,function(){n(TestState.INIT_DOWN),f.debug(t.id+": start short download"),p=v,a(t,S.pretestDurationMs)}),t.onStateEnter(TestState.PING,function(){n(TestState.PING),f.debug(t.id+": starting ping"),0===t.id?u(t):t.triggerNextState()}),t.onStateEnter(TestState.DOWN,function(){return n(TestState.DOWN),void t.triggerNextState()}),t.onStateEnter(TestState.CONNECT_UPLOAD,function(){n(TestState.INIT_UP),t.socket.onerror=d.IGNORE,t.socket.onclose=d.IGNORE,t.socket.close(),o(t,i,e.test_token,d.CALLGLOBALHANDLER)}),t.onStateEnter(TestState.INIT_UP,function(){p=v,c(t,S.pretestDurationMs)}),t.onStateEnter(TestState.UP,function(){if(n(TestState.UP),D.length>0){var s=r(D,S.uploadThreadsLimitsMbit,!0);if(navigator.userAgent.match(/Firefox\/82\.0.*/)){var o=RMBTTestResult.calculateOverallSpeedFromMultipleThreads(b.threads,function(e){return e.down}),a=o.speed/4,i=r([a/8],S.uploadThreadsLimitsMbit,!0);i.numThreads>s.numThreads&&(f.debug("overwriting for Firefox 82 to "+i.numThreads+" threads"),s=i)}A=s.numThreads,_&&(p=s.chunkSize),D=[]}t.id<A?g(t,e.test_duration):(t.socket.onerror=d.IGNORE,t.socket.onclose=d.IGNORE,t.socket.readyState!==WebSocket.CLOSED&&t.socket.close(),t.triggerNextState())}),t.onStateEnter(TestState.END,function(){t.socket.readyState!==WebSocket.CLOSED&&t.socket.close(),0===t.id&&s()}),t.setState(TestState.INIT),n(TestState.INIT),o(t,i,e.test_token,d.CALLGLOBALHANDLER)}function o(e,t,n,s){try{e.socket=new WebSocket(t)}catch(e){return void x(RMBTError.SOCKET_INIT_FAILED)}e.socket.binaryType="arraybuffer",e.socket.onerror=s,e.socket.onclose=s,e.socket.onmessage=function(t){if(0===t.data.indexOf("CHUNKSIZE")){var o=t.data.trim().split(" ");4===o.length?(h=parseInt(o[1]),v=parseInt(o[2]),m=parseInt(o[3])):(h=parseInt(o[1]),v=h),f.debug(e.id+": Chunksizes: min "+v+", max: "+m+", default: "+h)}else if(0===t.data.indexOf("RMBTv")){var r=t.data.substring(5).trim();S.client_version=r,0===r.indexOf("1.")?_=!0:0===r.indexOf("0.3")?_=!1:f.warn("unknown server version: "+r)}else"ACCEPT TOKEN QUIT\n"===t.data?e.socket.send("TOKEN "+n+"\n"):"OK\n"===t.data&&e.state===TestState.INIT?f.debug(e.id+": Token accepted"):"ERR\n"===t.data?(s(),f.error("got error msg")):0===t.data.indexOf("ACCEPT GETCHUNKS")&&e.triggerNextState()}}function r(e,t,n){C=e.reduce(function(e,t){return e+t}),f.debug("total: circa "+C/1e3+" KB/sec"),f.debug("total: circa "+8*C/1e6+" MBit/sec");var s=8*C/1e6,o=0;Object.keys(t).forEach(function(e){s>e&&(o=t[e])}),o=Math.min(B,o),f.debug("set number of threads to be used in upcoming speed test to: "+o);var r=C/(1e3/(S.measurementPointsTimespan/2));if(r-=r%1024,r=Math.max(v,r),r=Math.min(m,r),f.debug("calculated chunksize for upcoming speed test "+r/1024+" KB"),n){var a=Number.POSITIVE_INFINITY;Object.keys(P).forEach(function(e){Math.abs(r-e)<Math.abs(r-a)?("string"==typeof e&&(e=parseInt(e)),a=e):(delete P[e],delete y[e])}),r=a,f.debug("fallback to existing chunksize for upcoming speed test "+r/1024+" KB")}return{numThreads:o,chunkSize:r,bytesPerSecs:C}}function a(e,t){var n=e.socket.onmessage,s=nowMs(),o=1,r=0,a=p;!function u(){i(e,o,a,function(i){r+=o*a,f.debug(e.id+": "+i);var d=parseInt(i.substring(5));nowMs()-s>t?(D.push(o*a/(d/1e9)),e.socket.onmessage=n):o<8||!_?(o*=2,u()):(a=Math.min(2*a,m),u())})}()}function i(e,t,n,s){var o=e.socket,r=t,a=p*t,i=0,u=null,d=function(e,t){var n=new Uint8Array(e.length+t.length);return n.set(e,0),n.set(t,e.length),n},c=function(e){if("string"!=typeof e.data){var t=!1;u=null===u?new Uint8Array(e.data):d(u,new Uint8Array(e.data)),i+=e.data.byteLength,u.length===n&&(r--,t=!0),t&&255===u[u.length-1]?(o.onmessage=function(e){var t=e.data;s(t)},o.send("OK\n"),y[n]=u.buffer,u=null):(P.hasOwnProperty(n)||(P[n]=[]),t&&(P[n].length<S.savedChunks&&P[n].push(u.buffer),u=null))}};o.onmessage=c,f.debug(e.id+": downloading "+t+" chunks, "+a/1e3+" KB");var l="GETCHUNKS "+t+(n!==h?" "+n:"")+"\n";o.send(l)}function u(e){var t=e.socket.onmessage,n=S.numPings;d(e,function s(o){if(e.result.pings.push(o),n===S.numPings-1&&(M.durationPingMs=(e.result.pings[1].timeNs-e.result.pings[0].timeNs)/1e6*S.numPings,f.debug(e.id+": PING phase will take approx "+M.durationPingMs+" ms")),f.debug(e.id+": PING "+o.client+" ns client; "+o.server+" ns server"),--n>0)e.socket.onmessage=function(t){"ACCEPT GETCHUNKS GETTIME PUT PUTNORESULT PING QUIT\n"===t.data?d(e,s):f.error("unexpected error during ping test")};else{for(var r=[],a=0;a<e.result.pings.length;a++)r.push(e.result.pings[a].client);b.ping_client_median=Math.median(r),b.ping_client_shortest=Math.min.apply(Math,r);for(var i=[],u=0;u<e.result.pings.length;u++)i.push(e.result.pings[u].server);b.ping_server_median=Math.median(i),b.ping_server_shortest=Math.min.apply(Math,i),f.debug(e.id+": median client: "+Math.round(b.ping_client_median/1e3)/1e3+" ms; median server: "+Math.round(b.ping_server_median/1e3)/1e3+" ms"),f.debug(e.id+": shortest client: "+Math.round(b.ping_client_shortest/1e3)/1e3+" ms; shortest server: "+Math.round(b.ping_server_shortest/1e3)/1e3+" ms"),e.socket.onmessage=t}})}function d(e,t){var n=void 0,s=void 0,o=function(o){if("PONG\n"===o.data){var r=nowNs();s=r-n,e.socket.send("OK\n")}else if(0===o.data.indexOf("TIME")){var a=new RMBTPingResult;a.client=s,a.server=parseInt(o.data.substring(5)),a.timeNs=n,t(a)}};e.socket.onmessage=o,n=nowNs(),e.socket.send("PING\n")}function c(e,t){var n=e.socket.onmessage,s=nowMs(),o=1,r=0,a=p,i=1024,u=m;navigator.userAgent.match(/Firefox\/82\.0.*/)&&(u=524288,i=16),window.setTimeout(function(){var e=nowMs(),n=e-s;f.debug("diff:"+(n-t)+" ("+(n-t)/t+" %)")},t);!function d(){l(e,o,a,function(c){if(r+=o*a,f.debug(e.id+": "+c),nowMs()-s>t){e.socket.onmessage=n;var l=parseInt(c.substring(5));D.push(o*a/(l/1e9))}else{var g=Math.min(2*a,u);o<8||!y.hasOwnProperty(g)||!_?(o=Math.min(i,2*o),d()):(a=g,d())}})}()}function l(e,t,n,s){var o=e.socket;o.onmessage=function(r){if(0===r.data.indexOf("OK"))for(var a=0;a<t;a++){var i=void 0;if(a===t-1){var u=new Uint8Array(P[n][0].slice(0));u[u.length-1]=255,i=u.buffer}else i=P[n][0];var d=new Uint8Array(i);d[0]=a,d[1]=e.id,console.log("bytes 0 1 -3 -2 -1: "+e.id+" / "+a,d[0],d[1],d[d.length-3],d[d.length-2],d[d.length-1]),o.send(d.buffer)}else{if(0===r.data.indexOf("ACCEPT"))return;0===r.data.indexOf("ERR")?console.log("err"):s(r.data)}},f.debug(e.id+": uploading "+t+" chunks, "+n*t/1e3+" KB"),o.send("PUTNORESULT"+(_?" "+n:"")+"\n")}function g(e,t){var n=e.socket.onmessage,s=C/2/A,o=1.5*C/A,r=document.hidden?o:s,a=function(){r=document.hidden?o:s,f.debug("document visibility changed to: "+document.hidden)};document.addEventListener("visibilitychange",a);var i=Math.ceil(C/A/p),u=!1,d=!0,c=-1,l=0,g=function s(){u||(f.debug(e.id+": is 7.2 sec in, got data for "+c),c<1e9*t&&l<3e3?(window.setTimeout(s,250),l+=250):(f.debug(e.id+": didn't finish, timeout extended by "+l+" ms, last info for "+c),e.socket.onerror=function(){},e.socket.onclose=function(){},e.socket.close(),e.socket.onmessage=n,f.debug(e.id+": socket now closed: "+e.socket.readyState),document.removeEventListener("visibilitychange",a),e.triggerNextState()))},T=function t(){if(e.socket.bufferedAmount<r)for(var n=0;n<i;n++)e.socket.send(P[p][n%P[p].length]);if(!d)return!1;setTimeout(t,0)};window.setTimeout(g,1e3*t+200),window.setTimeout(function(){d=!1,e.socket.onclose=function(){},e.socket.send(y[p]),e.socket.send("QUIT\n")},1e3*t),f.debug(e.id+": set timeout");var m=1e6*S.measurementPointsTimespan,v=/TIME (\d+) BYTES (\d+)/,_=/TIME (\d+)/,I=function(t){"OK\n"===t.data&&T();var s=v.exec(t.data);if(null!==s){var o={duration:parseInt(s[1]),bytes:parseInt(s[2])};o.duration-c>m&&(c=o.duration,e.result.up.push(o))}else null!==(s=_.exec(t.data))&&(u=!0,f.debug("Upload duration: "+s[1]),e.socket.onmessage=n,document.removeEventListener("visibilitychange",a))};e.socket.onmessage=I,e.socket.send("PUT"+(p!==h?" "+p:"")+"\n")}function T(e){return{client_language:"de",client_name:S.client,client_uuid:S.uuid,client_version:S.client_version,client_software_version:S.client_software_version,geoLocations:b.geoLocations,model:S.model,network_type:98,platform:S.platform,product:S.product,pings:b.pings,test_bytes_download:b.bytes_download,test_bytes_upload:b.bytes_upload,test_nsec_download:b.nsec_download,test_nsec_upload:b.nsec_upload,test_num_threads:U,num_threads_ul:A,test_ping_shortest:b.ping_server_shortest,test_speed_download:b.speed_download,test_speed_upload:b.speed_upload,test_token:e.test_token,test_uuid:e.test_uuid,time:b.beginTime,timezone:S.timezone,type:"DESKTOP",version_code:"1",speed_detail:b.speedItems,user_server_selection:S.userServerSelection}}var f=log.getLogger("rmbtws"),p=null,m=4194304,v=0,h=4096,_=!1,S=void 0,I=void 0,b=null,N=null,k=null,E=TestState.INIT,w=null,M={durationInitMs:2500,durationPingMs:1e4,durationUpMs:-1,durationDownMs:-1},O=new RMBTIntermediateResult,R=[],P={},y={},L=null,B=0,U=0,A=0,D=[],C=0;this.onError=function(e){N=e},this.onStateChange=function(e){k=e};var x=function(e){if(f.debug("error occurred during websocket test:",e),e!==RMBTError.NOT_SUPPORTED&&n(TestState.ERROR),null!==N){var t=N;N=null,t(e)}};this.startTest=function(){if(void 0===window.WebSocket)return void x(RMBTError.NOT_SUPPORTED);n(TestState.INIT),b=new RMBTTestResult,I.getDataCollectorInfo(),I.obtainControlServerRegistration(function(e){B=parseInt(e.test_numthreads),B=1,L=new CyclicBarrier(B),M.durationDownMs=1e3*e.test_duration,M.durationUpMs=1e3*e.test_duration,null!==TestEnvironment.getTestVisualization()&&TestEnvironment.getTestVisualization().updateInfo(e.test_server_name,e.client_remote_ip,e.provider,e.test_uuid);var t=function(){f.debug("got geolocation, obtaining token and websocket address");var t=function(){n(TestState.INIT),b.beginTime=Date.now();for(var t=0;t<B;t++){var r=new RMBTTestThread(L);r.id=t,b.addThread(r.result),s(e,r,function(){f.info("All tests finished"),o.stop(),b.geoLocations=o.getResults(),b.calculateAll(),I.submitResults(T(e),function(){n(TestState.END)},function(){x(RMBTError.SUBMIT_FAILED)})}),R.push(r)}};0===e.test_wait?t():(f.info("test scheduled for start in "+e.test_wait+" second(s)"),n(TestState.WAIT),self.setTimeout(function(){t()},1e3*e.test_wait))},o=void 0;null!==TestEnvironment.getGeoTracker()?(o=TestEnvironment.getGeoTracker(),t()):(o=new GeoTracker,f.debug("getting geolocation"),o.start(function(){t()},TestEnvironment.getTestVisualization()))},function(){x(RMBTError.REGISTRATION_FAILED)})},this.getIntermediateResult=function(){O.status=E;var e=nowNs()/1e6-w;switch(O.status){case TestState.WAIT:O.progress=0;break;case TestState.INIT:case TestState.INIT_DOWN:case TestState.INIT_UP:O.progress=e/M.durationInitMs;break;case TestState.PING:O.progress=e/M.durationPingMs;break;case TestState.DOWN:O.progress=e/M.durationDownMs;break;case TestState.UP:O.progress=e/M.durationUpMs;break;case TestState.END:O.progress=1;break;case TestState.ERROR:case TestState.ABORTED:O.progress=0}if(isNaN(O.progress)&&(O.progress=0),O.progress=Math.min(1,O.progress),null!==b){if(O.status!==TestState.PING&&O.status!==TestState.DOWN||(O.pingNano=b.ping_server_median),O.status===TestState.DOWN||O.status==TestState.INIT_UP){var t=RMBTTestResult.calculateOverallSpeedFromMultipleThreads(b.threads,function(e){return e.down});O.downBitPerSec=t.speed,O.downBitPerSecLog=(Math.log10(O.downBitPerSec/1e6)+2)/4}if(O.status===TestState.UP||O.status==TestState.INIT_UP){var n=RMBTTestResult.calculateOverallSpeedFromMultipleThreads(b.threads,function(e){return e.up});O.upBitPerSec=n.speed,O.upBitPerSecLog=(Math.log10(O.upBitPerSec/1e6)+2)/4}}return O},this.getState=function(){return"INIT"},function(e,t){S=e,I=t}(e,t)}
