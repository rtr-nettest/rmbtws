 ...
 *****************************************************************************!*/
"use strict";var RMBTTest=function(){function e(e){S=e,_=TestState.INIT}function t(e){void 0!==_&&_===e||(_=e,h=performance.now())}function n(e,n,r){var i=(e.test_server_encryption?"wss://":"ws://")+e.test_server_address+":"+e.test_server_port;w(i);var u=function(){return{IGNORE:function(){},CALLGLOBALHANDLER:function(){D(RMBTError.CONNECT_FAILED)},TRYRECONNECT:function(){D(RMBTError.CONNECT_FAILED)}}}();n.onStateEnter(TestState.INIT_DOWN,function(){t(TestState.INIT_DOWN),w(n.id+": start short download"),g=v,0===n.id?o(n,S.pretestDurationMs):n.triggerNextState()}),n.onStateEnter(TestState.PING,function(){t(TestState.PING),w(n.id+": starting ping"),0===n.id?a(n):n.triggerNextState()}),n.onStateEnter(TestState.DOWN,function(){t(TestState.DOWN),n.id<B?d(n,e.test_duration):n.triggerNextState()}),n.onStateEnter(TestState.INIT_UP,function(){t(TestState.INIT_UP),g=v,0===n.id?c(n,S.pretestDurationMs):n.triggerNextState()}),n.onStateEnter(TestState.UP,function(){t(TestState.UP),n.id<C?l(n,e.test_duration):(n.socket.onerror=u.IGNORE,n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),n.triggerNextState())}),n.onStateEnter(TestState.END,function(){n.socket.readyState!==WebSocket.CLOSED&&n.socket.close(),0===n.id&&r()}),n.setState(TestState.INIT),t(TestState.INIT),s(n,i,e.test_token,u.CALLGLOBALHANDLER)}function s(e,t,n,s){try{e.socket=new WebSocket(t)}catch(e){return void D(RMBTError.SOCKET_INIT_FAILED)}e.socket.binaryType="arraybuffer",e.socket.onerror=s,e.socket.onmessage=function(t){if(0===t.data.indexOf("CHUNKSIZE")){var o=t.data.trim().split(" ");4===o.length?(m=parseInt(o[1]),v=parseInt(o[2]),E=parseInt(o[3])):(m=parseInt(o[1]),v=m),w(e.id+"Chunksizes: min "+v+", max: "+E+", default: "+m)}else if(0===t.data.indexOf("RMBTv")){var r=t.data.substring(5).trim();S.client_version=r,0===r.indexOf("1.")?b=!0:0===r.indexOf("0.3")?b=!1:w("unknown server version: "+r)}else"ACCEPT TOKEN QUIT\n"===t.data?e.socket.send("TOKEN "+n+"\n"):"OK\n"===t.data&&e.state===TestState.INIT?w(e.id+": Token accepted"):"ERR\n"===t.data?(s(),w("got error msg")):0===t.data.indexOf("ACCEPT GETCHUNKS")&&e.triggerNextState()}}function o(e,t){var n=e.socket.onmessage,s=nowNs()/1e6,o=1,a=0;!function i(){r(e,o,function(r){a+=o*g,w(e.id+": "+r);var d=parseInt(r.substring(5));if(nowNs()/1e6-s>t){k=o*g/(d/1e9),w(e.id+": circa "+k/1e3+" KB/sec"),w(e.id+": circa "+8*k/1e6+" MBit/sec");var c=8*k/1e6;Object.keys(S.downloadThreadsLimitsMbit).forEach(function(e){c>e&&(B=S.downloadThreadsLimitsMbit[e])}),B=Math.min(I,B),w(e.id+": set number of threads to be used in download speed test to: "+B);var u=k/(1e3/(S.measurementPointsTimespan/2));u-=u%1024,u=Math.max(v,u),u=Math.min(E,u),w(e.id+": calculated chunksize for download speed test "+u/1024+" KB"),g=u,e.socket.onmessage=n}else o<8||!b?(o*=2,i()):(g=Math.min(2*g,E),i())})}()}function r(e,t,n){var s=e.socket,o=t,r=g*t,a=0,i=function(e){"string"!=typeof e.data&&(a+=e.data.byteLength,e.data.byteLength!==g&&1!==e.data.byteLength||o--,0===o?(s.onmessage=function(e){var t=e.data;n(t)},s.send("OK\n"),U[g]=e.data):(L.hasOwnProperty(g)||(L[g]=[]),L[g].length<S.savedChunks&&L[g].push(e.data)))};s.onmessage=i,w(e.id+": downloading "+t+" chunks, "+r/1e3+" KB");var d="GETCHUNKS "+t+(g!==m?" "+g:"")+"\n";s.send(d)}function a(e){var t=1/0,n=e.socket.onmessage,s=S.numPings;i(e,function o(r){if(s--,e.result.pings.push(r),r.client<t&&(t=r.client),w(e.id+": PING "+r.client+" ns client; "+r.server+" ns server"),s>0)e.socket.onmessage=function(t){"ACCEPT GETCHUNKS GETTIME PUT PUTNORESULT PING QUIT\n"===t.data?i(e,o):w("unexpected error during ping test")};else{for(var a=[],d=0;d<e.result.pings.length;d++)a.push(e.result.pings[d].client);M.ping_median=Math.median(a),w(e.id+": shortest: "+Math.round(t/1e3)/1e3+" ms"),M.ping_shortest=t,e.socket.onmessage=n}})}function i(e,t){var n,s,o=function(o){if("PONG\n"===o.data){var r=nowNs();s=r-n,e.socket.send("OK\n")}else if(0===o.data.indexOf("TIME")){var a=new RMBTPingResult;a.client=s,a.server=parseInt(o.data.substring(5)),a.timeNs=n,t(a)}};e.socket.onmessage=o,n=nowNs(),e.socket.send("PING\n")}function d(e,t){var n,s,o=e.socket.onmessage,r=0,a=0,i=-1,d=null,c=null;n=window.setInterval(function(){if(null!==d&&i!==a){i=a;var t=nowNs();w(e.id+": "+s+"|"+S.measurementPointsTimespan+"|"+t+"|"+a);var u=new Uint8Array(d,d.byteLength-1,1),p=c-l;e.result.down.push({duration:p,bytes:r}),s=t,u[0]>=255&&(w(e.id+": received end chunk"),window.clearInterval(n),e.socket.onmessage=function(t){w(t.data),e.socket.onmessage=o},e.socket.send("OK\n"))}},S.measurementPointsTimespan);var u=function(e){a++,r+=e.data.byteLength,c=nowNs(),d=e.data};e.socket.onmessage=u;var l=nowNs();e.socket.send("GETTIME "+t+(g!==m?" "+g:"")+"\n")}function c(e,t){var n=e.socket.onmessage,s=nowNs()/1e6,o=1,r=0;window.setTimeout(function(){var e=nowNs()/1e6,n=e-s;w("diff:"+(n-t)+" ("+(n-t)/t+" %)")},t);!function a(){u(e,o,function(i){if(r+=o*g,w(e.id+": "+i),nowNs()/1e6-s>t){e.socket.onmessage=n;var d=parseInt(i.substring(5));k=o*g/(d/1e9),w(e.id+": circa "+k/1e3+" KB/sec up"),w(e.id+": circa "+8*k/1e6+" MBit/sec up");var c=8*k/1e6;Object.keys(S.uploadThreadsLimitsMbit).forEach(function(e){c>e&&(C=S.uploadThreadsLimitsMbit[e])}),C=Math.min(I,C),w(e.id+": set number of threads to be used in upload speed test to: "+C);var u=k/(1e3/(S.measurementPointsTimespan/2));u-=u%1024,u=Math.max(m,u),u=Math.min(E,u);var l=Number.POSITIVE_INFINITY;Object.keys(L).forEach(function(e){Math.abs(u-e)<Math.abs(u-l)?l=e:(delete L[e],delete U[e])}),w(e.id+": calculated chunksize for upload speed test "+u/1024+" KB"),w(e.id+": used chunk size for upload speed test will be: "+l/1024+" KB"),g=l}else{var p=2*g;o<8||!U.hasOwnProperty(p)||!b?(o*=2,a()):(g=Math.min(2*g,E),a())}})}()}function u(e,t,n){var s=e.socket;s.onmessage=function(e){0!==e.data.indexOf("OK")&&0!==e.data.indexOf("ACCEPT")&&n(e.data)},w(e.id+": uploading "+t+" chunks, "+g*t/1e3+" KB"),s.send("PUTNORESULT"+(b?" "+g:"")+"\n");for(var o=0;o<t;o++){var r;r=o===t-1?U[g]:L[g][0],s.send(r)}}function l(e,t){var n=e.socket.onmessage,s=k/2/C,o=Math.ceil(k/C/g),r=!1,a=!0,i=-1,d=0,c=function s(){r||(w(e.id+": is 7.2 sec in, got data for "+i),i<1e9*t&&d<3e3?(window.setTimeout(s,250),d+=250):(w(e.id+": didn't finish, timeout extended by "+d+" ms, last info for "+i),e.socket.onerror=function(){},e.socket.close(),e.socket.onmessage=n,w(e.id+": socket now closed: "+e.socket.readyState),e.triggerNextState()))},u=function t(){if(e.socket.bufferedAmount<s)for(var n=0;n<o;n++)e.socket.send(L[g][n%L[g].length]);if(!a)return!1;setTimeout(t,0)};window.setTimeout(c,1e3*t+200),window.setTimeout(function(){a=!1,e.socket.send(U[g]),e.socket.send("QUIT\n")},1e3*t),w(e.id+": set timeout");var l=/TIME (\d+) BYTES (\d+)/,p=/TIME (\d+)/,f=function(t){"OK\n"===t.data&&u();var s=l.exec(t.data);if(null!==s){var o={duration:parseInt(s[1]),bytes:parseInt(s[2])};i=o.duration,e.result.up.push(o)}else null!==(s=p.exec(t.data))&&(r=!0,w("Upload duration: "+s[1]),e.socket.onmessage=n)};e.socket.onmessage=f,e.socket.send("PUT"+(g!==m?" "+g:"")+"\n")}function p(e,t){var n={version:e.version,language:e.language,uuid:e.uuid,type:e.type,version_code:e.version_code,client:e.client,timezone:e.timezone,time:(new Date).getTime()};"undefined"!=typeof userServerSelection&&userServerSelection>0&&"undefined"!=typeof UserConf&&void 0!==UserConf.preferredServer&&"default"!==UserConf.preferredServer&&(n.prefer_server=UserConf.preferredServer,n.user_server_selection=userServerSelection),$.ajax({url:e.controlServerURL+e.controlServerRegistrationResource,type:"post",dataType:"json",contentType:"application/json",data:JSON.stringify(n),success:function(e){var n=new RMBTControlServerRegistrationResponse(e);t(n)},error:function(){w("error getting testID")}})}function f(e){$.ajax({url:e.controlServerURL+e.controlServerDataCollectorResource,type:"get",dataType:"json",contentType:"application/json",success:function(t){e.product=t.agent.substring(0,Math.min(150,t.agent.length)),e.model=t.product,e.os_version=t.version},error:function(){w("error getting data collection response")}})}function T(e,t){var n={client_language:"de",client_name:S.client,client_uuid:S.uuid,client_version:S.client_version,client_software_version:S.client_software_version,geoLocations:M.geoLocations,model:S.model,network_type:98,platform:S.platform,product:S.product,pings:M.pings,test_bytes_download:M.bytes_download,test_bytes_upload:M.bytes_upload,test_nsec_download:M.nsec_download,test_nsec_upload:M.nsec_upload,test_num_threads:B,num_threads_ul:C,test_ping_shortest:M.ping_shortest,test_speed_download:M.speed_download,test_speed_upload:M.speed_upload,test_token:e.test_token,time:M.beginTime,timezone:"Europe/Vienna",type:"DESKTOP",version_code:"1",speed_detail:M.speedItems,user_server_selection:S.userServerSelection},s=JSON.stringify(n);w("Submit size: "+s.length),$.ajax({url:S.controlServerURL+S.controlServerResultResource,type:"post",dataType:"json",contentType:"application/json",data:s,success:function(n){w("https://develop.netztest.at/en/Verlauf?"+e.test_uuid),t()},error:function(){w("error submitting results")}})}var g,v,m,S,_,h,N,I,k,w=function(e){$("#debug").prepend(e+"\n"),console.log(e)},E=4194304,b=!1,M=null,O=null,y={durationInitMs:2500,durationPingMs:500,durationUpMs:-1,durationDownMs:-1},R=new RMBTIntermediateResult,P=[],L={},U={},B=0,C=0;e.prototype.onError=function(e){O=e};var D=function(e){if(!e===RMBTError.NOT_SUPPORTED&&t(TestState.ERROR),null!==O){var n=O;O=null,n()}};return e.prototype.startTest=function(){if(void 0===window.WebSocket)return void D(RMBTError.NOT_SUPPORTED);t(TestState.INIT),M=new RMBTTestResult,f(S),p(S,function(e){I=parseInt(e.test_numthreads),N=new CyclicBarrier(I),y.durationDownMs=1e3*e.test_duration,y.durationUpMs=1e3*e.test_duration,null!==TestEnvironment.getTestVisualization()&&TestEnvironment.getTestVisualization().updateInfo(e.test_server_name,e.client_remote_ip,e.provider,e.test_uuid);var s,o=function(){w("got geolocation, obtaining token and websocket address"),t(TestState.WAIT),window.setTimeout(function(){t(TestState.INIT),M.beginTime=Date.now();for(var o=0;o<I;o++){var r=new RMBTTestThread(N);r.id=o,M.addThread(r.result),n(e,r,function(){w("All tests finished"),s.stop(),M.geoLocations=s.getResults(),M.calculateAll(),T(e,function(){t(TestState.END)})}),P.push(r)}},1e3*e.test_wait)};null!==TestEnvironment.getGeoTracker()?(s=TestEnvironment.getGeoTracker(),o()):(s=new GeoTracker,w("getting geolocation"),s.start(function(){o()}))})},e.prototype.getIntermediateResult=function(){R.status=_;var e=nowNs()/1e6-h;switch(R.status){case TestState.WAIT:R.progress=0;break;case TestState.INIT:case TestState.INIT_DOWN:case TestState.INIT_UP:R.progress=e/y.durationInitMs;break;case TestState.PING:R.progress=e/y.durationPingMs;break;case TestState.DOWN:R.progress=e/y.durationDownMs;break;case TestState.UP:R.progress=e/y.durationUpMs;break;case TestState.END:R.progress=1;break;case TestState.ERROR:case TestState.ABORTED:R.progress=0}if(isNaN(R.progress)&&(R.progress=0),R.progress=Math.min(1,R.progress),null!==M){if(R.pingNano=M.ping_median,R.status===TestState.DOWN){for(var t=0,n=1/0,s=0;s<P.length>0;s++){var o=M.threads[s].down;o.length>0&&(t+=o[o.length-1].bytes,o[o.length-1].duration<n&&(n=o[o.length-1].duration))}R.downBitPerSec=Math.max(0,8*t/(n/1e9)),R.downBitPerSecLog=(log10(R.downBitPerSec/1e6)+2)/4}if(R.status===TestState.UP){t=0,n=1/0;for(var s=0;s<P.length;s++){var r=M.threads[s].up;r.length>0&&(t+=r[r.length-1].bytes,r[r.length-1].duration<n&&(n=r[r.length-1].duration))}R.upBitPerSec=Math.max(0,8*t/(n/1e9)),R.upBitPerSecLog=(log10(R.upBitPerSec/1e6)+2)/4}}return R},e.prototype.getState=function(){return"INIT"},e}();
